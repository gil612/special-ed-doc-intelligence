...
(דרך `@opennextjs/cloudflare`, deployed כ-Worker רגיל, לא Pages) — אין שירות
backend נפרד. Workers לא מריצים Python, כך שה-pipeline ב-production הוא
port ל-TypeScript של הלוגיקה שפותחה ב-`iep_schema.py`/`redaction.py`; שני
קבצי הפייתון האלו נשארים בריפו כ-reference implementation מקומי בלבד
(עיצוב סכימה/פרומפט בלי להריץ deploy), ולא כמה שרץ בפועל.

הפרויקט התחיל עם `@cloudflare/next-on-pages` (Cloudflare Pages), אך נתקל
בבאג אמיתי בשכבת ה-Request wrapping שלו (`TypeError: Illegal invocation`
על `request.formData()`, שחזור רק תחת ה-Workers runtime האמיתי, לא בטסטים
מקומיים) — Cloudflare עצמה מציינת ש-`@cloudflare/next-on-pages` deprecated
לטובת `@opennextjs/cloudflare`, שאין לו את הבעיה הזו.
...
Workers edge runtime. אותו מודל ואותה תמיכת Structured Output; ההבדל הוא
קו המוצר/חיוב ב-Google Cloud, לא היכולת. שימוש ב-`gemini-flash-latest`
(alias יציב) ולא בגרסה מוצמדת כמו `gemini-2.5-flash` — גרסאות מוצמדות
יכולות להיות עדיין רשומות ב-API אך לדחות קריאות `generateContent` בפועל
(404), כפי שקרה בפרויקט הזה.
...
context.waitUntil(...) — ממשיך לרוץ באותה Worker invocation:
    ...
    3. Gemini API (gemini-flash-latest) — Structured Output לפי IEPExtraction (Zod)
...
## עלות תפעולית — 100 מסמכים ליום

**מתודולוגיה:** אורך טקסט נמדד בפועל (לא ניחוש) משני מקורות: תבנית ה-prompt
עצמה (`buildExtractionPrompt`, 940 תווים קבועים) + טקסט מסמך מייצג
(`sample_iep_decision.pdf`, כ-1,400 תווים לאחר חילוץ). יחס תווים-לטוקן
לעברית הוערך כ-2.2 תווים/טוקן (יחס טיפוסי לעברית מול טוקנייזרים מבוססי-BPE,
פחות יעיל מאנגלית). התפוקה (JSON מובנה) נאמדה מתוך תגובות אמיתיות שנצפו
במהלך פיתוח הפרויקט (`data/accuracy_test_report.md`).

| פרמטר | ערך | מקור |
|---|---|---|
| טוקני קלט למסמך | ~1,100 | (940+1,400 תווים) ÷ 2.2 |
| טוקני פלט למסמך | ~300 | נאמד מתגובות JSON אמיתיות |
| מודל | `gemini-3.1-flash-lite` | `lib/gemini.ts` |
| מחיר קלט (paid tier) | $0.25 / 1M טוקנים | [Google AI pricing](https://ai.google.dev/gemini-api/docs/pricing) |
| מחיר פלט (paid tier) | $1.50 / 1M טוקנים | שם |

**חישוב חודשי (100 מסמכים/יום × 30 ימים = 3,000 מסמכים/חודש):**

```
קלט:  3,000 × 1,100 טוקנים = 3,300,000 טוקנים  →  3.3M × $0.25 = $0.83
פלט:  3,000 ×   300 טוקנים =   900,000 טוקנים  →  0.9M × $1.50 = $1.35
────────────────────────────────────────────────────────────────
סה"כ Gemini API:                                            ≈ $2.18/חודש
```

**תשתית (Cloudflare Workers, R2, Supabase):** בנפח הזה כל שלושת השירותים
נשארים בתוך המכסה החינמית — Workers Free כולל 100,000 בקשות/יום (פי 1,000
מהנדרש כאן), R2 Free כולל 10GB אחסון (100 מסמכים/יום × ~500KB ממוצע = כ-1.5GB
לחודש, כך שהמכסה תיחצה רק לאחר כ-6-7 חודשי צבירה), Supabase Free כולל
500MB DB — השורות הנשמרות הן מטא-דאטה מובנה בלבד (ה-PDF עצמו ב-R2, לא
ב-Postgres), כך שהמכסה רחוקה מלהתמלא. **עלות תשתית בפועל: $0/חודש.**

**סה"כ עלות תפעולית משוערת ב-100 מסמכים/יום: ~$2-3 לחודש**, כמעט כולה
Gemini API — לא תשתית Cloudflare/Supabase.

**הערה חשובה שעלתה מבדיקה בפועל:** מסמך אמיתי שנסרק (`cases/gil_2010.pdf`)
חולץ ל-7 תווים בלבד — משמע: מסמכים סרוקים אמיתיים (ללא שכבת טקסט/OCR
מוטמעת) לא בהכרח מכילים טקסט הניתן לחילוץ על ידי `pypdf`/`unpdf` בכלל,
ללא קשר לסוג המסמך. זהו מגבלה אמיתית של הפרויקט בהיקפו הנוכחי (אין OCR),
לא רק עניין של "סוג מסמך לא מתאים" — ותוספת OCR (Gemini vision multimodal
או Cloud Vision API נפרד) תוסיף עלות נוספת לחישוב זה אם תתווסף בעתיד.
